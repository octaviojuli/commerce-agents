...
def build_shopping_config(*, live: bool = False) -> ShoppingAgentConfig:
    ...
    return ShoppingAgentConfig(
        ...
        domain_search_notes=(
            ...
            "Every search must carry filters.attributes: destination — the customer's own word for "
            "where (欧洲, 斯里兰卡, 德法意瑞), never a 线路系 or a country you add to it, and the query "
            "carries no place the customer did not name; depart_from and depart_to "
            ...
            "no_shopping=yes when THIS customer wants 纯玩; hotel_level (四钻, 五钻) when a "
            "standard is named; "
            "departure_city ONLY for a 口岸 the customer or the advisor chose, and never for "
            "the city the customer lives in — a 线路's 出发城市 is the 口岸 its international "
            "flight boards at (上海, 成都, 重庆, 昆明), and a customer in 厦门 or 青岛 is flown "
            "to one of those on a 联运 leg, so saying where they live rules out no line at "
            "all: search without it, lead with the lines whose connecting attribute says "
            "可全国联运, and tell the advisor which 口岸 each one boards at and that the 联运 "
            "段 is the 门店's to confirm. Never assume a 口岸 the customer did not name; "
            "family=yes when "
            "children travel, which sorts the 亲子 lines first rather than dropping the others; "
            "region when the customer or the advisor names a 线路系 (德法意瑞, 法意瑞, 西欧多国, 英爱, 西葡, 北欧, "
            ...
            "meets the filter when it matches any one of them. "
            "Every filter is a thing THIS customer said in THIS conversation. A habit on file "
            "about the advisor (their customers usually depart from 上海, they lean towards 纯玩) "
            "is the advisor's, not this customer's: it never becomes a filter and never goes in "
            "the 已知 line. Apply it the way 亲子 is applied — let it order what you show, and "
            "name the matching chips in present_focus's preselect so the advisor confirms or "
            "drops it in one tap. A customer who is silent about 购物 or about which city they "
            "leave from has not asked for either. destination is matched against a line's countries, its 线路系, its "
            ...
            "your own, and the question is about ONE dimension: the one the block names under "
            "建议追问, which is the first thing the customer has not said yet. What the block "
            "lists under 已知 the customer has said — never ask it again, and never offer "
            "chips on it; the card shows it locked. (2) The request narrows to five lines or fewer and the result carries no "
            ...
            "(出发口岸, 酒店标准, 纯玩, 特色): do not choose for the customer — say in a sentence "
            ...
            "to region when the value is a 线路系; 天数 to days_min and days_max; 出发口岸 to "
