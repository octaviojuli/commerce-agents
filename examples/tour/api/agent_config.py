...
def build_shopping_config(*, live: bool = False) -> ShoppingAgentConfig:
    ...
    return ShoppingAgentConfig(
        ...
        domain_search_notes=(
            ...
            "a shortlist over that overview is held. A route record carrying attachment names its "
            "行程附件 (.docx or .pdf). When — and only when — the advisor asks for a line's "
            "itinerary document, 行程单 or attachment, call present_attachments with that "
            "line's id and the card offers the download; do not offer it unasked, do not say "
            "you cannot send files, and never paste a URL. A record carrying line_type (包团, 会销, 定制) is not on "
