...
def build_shopping_config(*, live: bool = False) -> ShoppingAgentConfig:
    ...
    return ShoppingAgentConfig(
        ...
        domain_search_notes=(
            ...
            "行程附件 (.docx or .pdf), which the advisor downloads from that route's card in the "
            "workbench; you cannot send files, so when asked for the itinerary document say it "
            "is on the card and never paste a URL. A record carrying line_type (包团, 会销, 定制) is not on "
