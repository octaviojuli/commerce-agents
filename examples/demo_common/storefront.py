...
class StorefrontHost:
    ...
    def __init__(
        ...
        *,
        ...
        # In memory unless the vertical hands over a store of its own (a subclass over
        # durable storage, as ``sessions.py`` describes).
...
def build_storefront_host(
    *,
    ...
    ``sessions`` puts a store of the vertical's own where the in-memory one would be."""
    host = StorefrontHost(
        title=title,
        backend=backend,
        agent=agent,
