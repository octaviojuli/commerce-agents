...
| [`examples/tour/`](examples/tour/) ACME 旅行社 | Advisor-side group-tour sales in Chinese: routes as families, dated departures as variants, holds that are 预留 orders on the agency's own ERP with a thirty-minute countdown, a `present_shortlist` share link, an `ErpClient` seam with mock and HTTP clients | — |
